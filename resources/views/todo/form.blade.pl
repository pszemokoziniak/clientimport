...
<div class="container">
    ...
    <form action="zadaniaform" method="post" id="form">
        @csrf
        ...
        <div class="jumbotron">
            ...
            <div class="col-md form-group row">
                ...
                <div class="col-4">
                    <label for="doKiedy" class="lead">Do kiedy</label><i class="fas fa-plus-square fa-2x ml-2 float-right" id="nip_btn" style="color:red; cursor:pointer;"></i>
                    <input class="w-40 form-control" type="data" id="doKiedy" name="doKiedy" value="{{ old('doKiedy') }}">
                    @error('doKiedy')
                    <div class="alert-danger">{{$message}}</div>
                    @enderror
                </div>
            ...
            <div class="col-md form-group">
                ...
                <input class="w-100 form-control" id="wykonawca" type="text" name="wykonawca" value="{{ old('wykonawca') }}">
                
                    <select class="w-50 form-control" name="wykonawca">
                        <option value="wybierz">wybierz</option>

                        @foreach($users as $user)
                        <option value="{{ $user['id'] }}" {{ (old("wykonawca") == $user['id'] ? "selected":"") }}>{{ $user['name'] }}</option>
            ...
            <div class="col-md form-group">
                ...
                <input class="w-100 form-control" id="status" type="text" name="status" value="{{ old('status') }}">
            ...
            <div class="col-md form-group row">
                <div class="col">
                    ...
                    <input class="w-100 form-control" type="text" id="opis" name="opis" value="{{ old('opis') }}">
                    @error('opis')
                    <div class="alert-danger">{{$message}}</div>
                    @enderror
                </div>
            ...
            <div class="col-md form-group">
                ...
                <input class="w-100 form-control" id="zleceniodawca" type="text" name="zleceniodawca" value="{{ old('zleceniodawca') }}">
                
                    <select class="w-50 form-control" name="zleceniodawca">
                        <option value="wybierz">wybierz</option>

                        @foreach($users as $user)
                        <option value="{{ $user['id'] }}" {{ (old("zleceniodawca") == $user['id'] ? "selected":"") }}>{{ $user['name'] }}</option>
                        @endforeach
                    </select>
