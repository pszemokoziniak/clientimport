...
<div class="container">
    ...
    <hr>
        <h3>Klient: {{$client['nazwa']}}<br /> Telefon: {{$client['nrtelefonu']}}<br /> Email: {{$client['email']}}</h3>
    ...
    <form action="/oferta" method="post" id="form">
        ...
        <div class="jumbotron">
            ...
            <input type="hidden" value="{{ request()->route('id') }}" name="id_client">
                ...
                <button type="submit" id="save" class="btn btn-success btn-block">Wyślij Ofertę</button>
        </div>
    </form>
