...
<div class="container">
    ...
    <form action="/dostawcaPradEdit" method="POST">
    @csrf
        <input type="hidden" name="id" value="{{$data['id']}}">
        <fieldset>
            <legend>Dostawca Prądu</legend>
            <div class="form-group">
            <label for="firmadostawca">Dostawca</label>
            <input type="text" class="form-control w-50" value="{{$data['firmadostawca']}}" id="firmadostawca" name="firmadostawca" aria-describedby="statusHelp" placeholder="wymagane">
            @error('firmadostawca')
